...
var zgetArrayStrClean = (arr) => {
    ...
    arr.forEach((el, i) => {
        let value = el;
        if (zisObject(value)) {
            value = zgetObjectStrClean(value);
        } else if (zisArray(value)) {
            value = zgetArrayStrClean(value);
        }
        if (i === arr.length - 1) {
            resStr += `${value}]`;
        } else resStr += `${value}, `;
...
var zremoveElement = (arg) => {
    ...
    else console.warn('zremoveEl > cant remove el for arg: ', arg, 'el: ', el);
};
...
var traverseElSibling = (currEl, argFn, direction) => {
    if (currEl.previousElementSibling) {
        argFn(currEl.previousElementSibling);
        traverseElSibling(currEl.previousElementSibling, argFn);
    }
};
...
var zge = zgetElement;
var zgae = zgetAllElements;
var zgaes = zgetAllElementsIncludingShadowRootsBySelector;
var zdlt = zDebugLogTrue;
var zdwt = zDebugWarnTrue;
var zdlp = zdebugLogPromise;
var zges = zgetElementAndScrollIntoView;
var zce = zclickElement;
var zca = zclickAll;
var zpm = zperformanceMeasure;
var zpma = zperformanceMeasureAsync;
var zmk = zmapKey;
var zmf = zmapFunction;
var zmkf = zmapKeyFunction;
